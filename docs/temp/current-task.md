...
| 2026-06-07 | **Navigation restructure (approved, NOT pushed):** bottom nav → Home/Sell/Inventory/Reports/More. New `/` Home dashboard (today sales·profit, low-stock, debt, quick actions), POS moved to `/sell`, `/more` hub, `/expenses` (expense mgmt moved out of Reports → Reports «ចំណេញ» now read-only profit + link), placeholders `/staff` `/suppliers`. Theme/UI unchanged. |
| 2026-06-07 | **Built Hold/Resume/Reprint/Receipt-History (fully functional, NOT pushed):** Dexie v6 `heldInvoices` table + `heldInvoice.service` (multiple drafts, local); cart store `setCart` (resume restores full cart); Hold button in CartPanel + held-count badge in POS header + `HeldInvoicesSheet`. Persisted `Sale.receiptNumber` (+`makeReceiptNumber`); `buildReceipt.ts` reconstructs ReceiptData from a stored sale (derives discount/change/remaining); `ReprintReceipt` component; reprint button in SaleDetailSheet; new `/receipts` Receipt History (search by receipt number + date, tap → reprint). backup clearAll includes heldInvoices. Verified end-to-end in preview (mobile). Typecheck ✅. ⚠️ NOT committed/pushed per founder. |
| 2026-06-07 | Created `PROJECT_AUDIT.md` (root) — full project analysis (MVP ~88%, prod-ready ~40%). **Built Expense module** (was listed missing in audit): Dexie v5 `expenses` table, Expense type/ExpenseId, `expense.service` (create/update/soft-delete + sync queue), `ExpenseFormSheet` (amount ៛/$, 6 categories, date, note, edit+soft-delete), and a 3rd "ចំណាយ" view in Reports showing ចំណូល/ចំណាយ/**ចំណេញសុទ្ធ** (net profit) + grouped list + FAB. backup export/restore/clearAll now include expenses. Verified ($5→20,000៛, net −5,000៛, edit/delete). Pushed to `main`. |
| 2026-06-06 | **Single product-name field (Khmer + English)** ([ProductFormSheet.tsx](apps/web/src/features/inventory/components/ProductFormSheet.tsx)): removed the separate "ឈ្មោះ English" input + `nameEn` state/payloads; one name box now holds both languages (placeholder "អង្ករ ២គីឡូ / Rice 2kg"). `productMatchesQuery` already searches name substring, so either language matches. Verified: "អង្ករ", "milo", "fanta" all find their item. Existing products keep nameEn (still searched). Pushed to `main`. |
| 2026-06-06 | **Reports tab shows $ alongside ៛** ([reports/page.tsx](apps/web/app/(app)/reports/page.tsx), [ReportExportSheet.tsx](apps/web/src/features/reports/components/ReportExportSheet.tsx)): added USD to top products, debt summary, history total bar, daily group totals, each sale row, payment-method breakdown — plus the Export/share image, preview, and share text. Summary cards already had $. Verified in overview + history. Pushed to `main`. |
